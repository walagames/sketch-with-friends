onmessage = function (e) {
    ...
    switch (workerData.action) {
      ...
      case "process":
        ...
        console.log("processImageAction complete");
        break;
      default:
  ...
  function fillImage(
    ...
    while ((point = candidatePoints.shift())) {
      ...
      if (!visited[visitedKey]) {
        ...
        // Check if the current pixel matches the fill color
        const currentPixelMatchesFillColor = 
            currentImageData[pointIdx] === r &&
            currentImageData[pointIdx + 1] === g &&
            currentImageData[pointIdx + 2] === b &&
            currentImageData[pointIdx + 3] === (forceSetAlphaValue || 255);
  
        const currentPointIsWhite = isWhite(pointIdx);
        // Don't fill if the pixel already matches our target color
        let canFill = (sourceImageData[alphaIdx] < 255 || currentPointIsWhite) && 
                     !currentPixelMatchesFillColor;
